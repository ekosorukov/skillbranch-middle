...
class ExampleUnitTest {
    ...
    @Test
    fun import_users_success() {
        val holder = UserHolder
        val raw = " John Doe ;JohnDoe@unknow.com;[B@7591083d:c6adb4becdc64e92857e1e2a0fd6af84;;"
        val importUsers = holder.importUsers(listOf(raw))
    ...
    fun drop_last_until_success() {
        val numbers = listOf(1, 2, 3).dropLastUntil { it == 2 }
        Assert.assertEquals(1, numbers.size)
        Assert.assertEquals(1, numbers[0])


        val gots = "House Nymeros Martell of Sunspear".split(" ").dropLastUntil { it == "of" } // [House, Nymeros, Martell])
        Assert.assertEquals(3, gots.size)
        Assert.assertEquals("House", gots[0])
        Assert.assertEquals("Nymeros", gots[1])
        Assert.assertEquals("Martell", gots[2])
